...
export function MapView() {
  ...
  }, [searchData, initialDataLoaded, setDateTimeFilters, setSearchQuery, handleGeocode]);

  const allSpots = useMemo(() => allGarages.flatMap((g: Garage) => g.spots || []), [allGarages]);
  const filteredSpots = useMemo(() => filterParkings(allSpots, filters), [allSpots, filters]);

  const filteredGarages = useMemo(() => {
    return allGarages.filter((g: Garage) =>
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      <div
        className="md:hidden fixed left-1/2 -translate-x-1/2 z-30 flex bg-card/90 backdrop-blur-md border border-border p-1 rounded-full shadow-2xl"
        style={{ bottom: 'calc(env(safe-area-inset-bottom) + 1rem)' }}
      >
        <button
          onClick={() => setView('map')}
          className={cn(
            "flex items-center gap-2 px-7 rounded-full text-[15px] font-semibold transition-all min-h-[48px]",
            view === 'map'
