...
export function Header() {
  ...
  const transparent = isHome && isAtTop;
          ...
          <div className="flex items-center md:hidden">
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              <DialogTrigger asChild>
                <Button variant="ghost" size="icon" className="md:hidden min-h-[56px] min-w-[56px]" style={transparent ? { color: 'white' } : {}}>
              ...
              <DialogContent className="w-full max-w-none h-auto flex flex-col p-6 top-0 left-0 translate-x-0 translate-y-0 border-t-0 rounded-b-[2.5rem] shadow-2xl data-[state=open]:slide-in-from-top data-[state=closed]:slide-out-to-top duration-300">
                <DialogHeader className="flex flex-row items-center justify-between mb-3">
                  <DialogTitle className="text-left text-xl font-bold">Menú Principal</DialogTitle>
                </DialogHeader>

                <div className="flex flex-col py-1 space-y-2">
                  {authUser ? (
                    <div className="flex items-center gap-4 px-4 py-4 bg-muted/50 rounded-2xl mb-1">
                      <UserAvatar size="h-12 w-12" />
                      <div className="min-w-0">
                        <p className="font-bold text-[16px] leading-tight">{authUser.user?.name || 'Usuario'}</p>
                        <p className="text-[13px] text-muted-foreground truncate mt-0.5">{authUser.user?.email}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-3 mb-1">
                      ...
                      <Button
                        ...
                        className="h-14 text-[16px] font-bold bg-primary text-white"
                      >
                        Registrarse
                      </Button>
                    </div>
                  ...
                  <div className="grid grid-cols-1">
                    {navigation.map((item) => (
                      <button
                        key={item.name}
                        onClick={() => handleNavigate(item.href)}
                        className="flex items-center w-full px-4 text-[17px] font-semibold active:bg-muted hover:bg-muted rounded-xl transition-colors min-h-[58px]"
                      >
                        {item.name}
                      </button>
                    ))}
                ...
                {authUser && (
                  <div className="mt-4 pt-4 border-t border-border">
                    <Button
                      variant="exit"
                      onClick={logout}
                      className="w-full h-14 text-[16px] justify-center text-destructive bg-destructive/5 hover:bg-destructive hover:text-white rounded-xl transition-all font-bold"
                    >
                      <LogOut className="h-5 w-5" />
                      Cerrar sesión
                    </Button>
