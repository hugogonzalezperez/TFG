...
Desde el aspecto técnico, para implementar el sistema se ha usado una tecnología que busca ser eficiente y escalable. Se utilizaron React y Vite para crear la parte de frontend y Supabase como servicio de backend. Se destaca la integración de Row Level Security (RLS) para garantizar la integridad y privacidad de los datos, así como el uso de Leaflet para la visualización de mapas interactivos. El resultado final es una solución robusta y accesible que optimiza el aprovechamiento del espacio urbano existente.

\begin{keywords}[Palabras clave]
Marketplace, P2P, aparcamiento, react, vite, supabase, economía colaborativa, reservas.
\end{keywords}
\end{abstract}
...
From a technical perspective, to implement the system a technology stack focused on efficiency and scalability has been used. React and Vite were used to create the frontend and Supabase as the backend service. The integration of Row Level Security (RLS) is highlighted to guarantee the integrity and privacy of the data, as well as the use of Leaflet for the visualization of interactive maps. The final result is a robust and accessible solution that optimizes the use of the existing urban space.

\begin{keywords}
Marketplace, P2P, parking, React, Vite, Supabase, sharing economy, reservations.
\end{keywords}
\end{abstract}
