...
\usepackage[colorlinks=true, linkcolor=black, citecolor=teal,
            urlcolor=teal, filecolor=teal]{hyperref}
...
\newcommand{\HRule}{\rule{\linewidth}{0.3mm}}
{
    ...
    \HRule\vspace*{-2mm}
    \begin{flushright}
        {\fontsize{32pt}{32pt}\selectfont Parky - Marketplace colaborativo de aparcamiento Peer-to-Peer\par
            \vspace*{3mm}
...

{
    ...
    \bigskip
    \begin{quote}
        \textit{``Parky - Marketplace colaborativo de aparcamiento Peer-to-Peer''}
    \end{quote}
...
\begin{abstract}
    ...
    Desde el aspecto técnico, el sistema se ha desarrollado con React~18 como \textit{framework} \textit{frontend}, TypeScript en modo estricto y Tailwind CSS para el diseño de interfaz, empleando Vite como herramienta de compilación y Supabase como backend gestionado, que proporciona base de datos PostgreSQL, autenticación con OAuth social y almacenamiento de ficheros. La seguridad de los datos se garantiza mediante políticas de \textit{Row Level Security} (RLS) y \textit{triggers} PL/pgSQL que aseguran la integridad de las reservas a nivel de base de datos. La búsqueda y localización de plazas se implementan mediante Leaflet y el servicio de geocodificación OpenCage. El sistema incluye un módulo de acceso físico digital (\textit{SmartAccess}) vinculado al estado de cada reserva, y una arquitectura de pagos P2P mediante Stripe Connect que contempla la gestión del IGIC canario. La aplicación se despliega simultáneamente como \textit{Single Page Application} en Vercel y como aplicación Android nativa mediante Capacitor.

    \begin{keywords}[Palabras clave]
        Marketplace colaborativo, P2P, aparcamiento urbano, React, TypeScript, Supabase, PostgreSQL, Stripe Connect, Capacitor, Row Level Security.
    \end{keywords}
\end{abstract}
...
\begin{abstract}[Abstract]
    Over the past decade, the increase in the number of vehicles and, above all, in tourism in many Canary Islands cities has made traffic and finding parking more difficult, especially in urban and tourist areas. This leads to problems such as traffic congestion, wasted time for drivers, and inefficient use of available space. At the same time, numerous private parking spaces remain empty for long periods of time.

    In this regard, digital platforms based on the sharing economy have become a useful tool for connecting people who have a resource to offer with those who need it (e.g., Airbnb, BlaBlaCar, Wallapop).

    Therefore, this Final Degree Project proposes the development of a full-stack application prototype for the rental of private parking spaces. The platform will operate as a marketplace connecting parking space owners (private homes or communities) with drivers who need to park their vehicles, facilitating the management of reservations and the availability of parking spaces.

    From a technical standpoint, the system has been developed using React~18 as the frontend framework, TypeScript in strict mode, and Tailwind CSS for interface design, employing Vite as the build tool, which provides a PostgreSQL database, social OAuth authentication, and file storage. Data security is ensured through \textit{Row Level Security} (RLS) policies and PL/pgSQL \textit{triggers} that guarantee the integrity of reservations at the database level. Searching for and locating parking spaces is implemented using Leaflet and the OpenCage geocoding service. The system includes a digital physical access module (\textit{SmartAccess}) linked to the status of each reservation, and a P2P payment architecture using Stripe Connect that handles the Canary Islands IGIC tax. The application is deployed simultaneously as a \textit{Single Page Application} on Vercel and as a native Android app using Capacitor.

    \begin{keywords}
        Collaborative marketplace, P2P, urban parking, React, TypeScript, Supabase, PostgreSQL, Stripe Connect, Capacitor, Row Level Security.
    \end{keywords}
\end{abstract}
...
\input{03-capítulo3}
\input{04-capítulo4}
\input{05-implementacion}
%\input{06-pruebas}
\input{06-conclusiones}
\input{07-conclusions}
\input{08-presupuesto}
